...
public partial class ManagerScript : Node2D
{
	...
	public override void _Process(double delta)
	{
		...
		Vector2 h = snakeScript.headPos;
		outOfBounds = (h.X > screenSize.X || h.X < -screenSize.X) || 
			(h.Y > screenSize.X || h.Y < -screenSize.Y) ||
			(h.Y > screenSize.X || h.Y < -screenSize.Y);
		if (Input.IsActionJustPressed("quit") || outOfBounds) {
